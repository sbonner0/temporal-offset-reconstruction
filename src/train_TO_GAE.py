...
from utils import (dotdict, eval_on_new_edges_only, evaluate_on_testset,
                   load_data, make_sparse, plot_results, save_model, evaluate_model)

def main(args):
    """ Train GAE Via Temporal Off-Set Reconstruction """ 
    ...
    print("Train GAE Via Temporal Off-Set Reconstruction")
    print(f'Using {args.dataset_str} dataset')

    adj, features = load_adj_graph(f'data/{args.dataset_str}.npz') # Load the original graph #NORMAL
    time_adj, time_features = load_adj_graph(f'data/{args.dataset_str}_0_rewire.npz')    #NORMAL

    # Store original offset adjacency matrix (without diagonal entries)
    ...
    for epoch in range(args.num_epochs):
        ...
        # Test loss used for diagram plotting
    ...
    accuracy, roc_score, ap_score = evaluate_on_testset(gae, test_edges, test_edges_false, data)
    print("Running on Testset From Training Time Snapshot")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', type=str, default='TO_GAE',
                        help='Model name.')  
